...
#     :param lsd1: train set's latent space data
#     :param lsd2: test set's latent space data
#     :param label: label of train set (assumed to be 0-indexed)
#     :return: Predicted label
#     """
#     F_h_h = np.dot(lsd2, np.transpose(lsd1))
...
#     # Si labels commencent à 0, pas de conversion
#     if label_min == 0:
#         label = label.reshape(len(label), 1)
#         enc = OneHotEncoder()
#         a = enc.fit_transform(label)
...
#         F_h_h_sum = np.dot(F_h_h, label_onehot)
#         F_h_h_mean = F_h_h_sum / label_num
#         label_pre = np.argmax(F_h_h_mean, axis=1)  # Pas de +1
    
#     # Si labels commencent à 1, faire la conversion
#     else:
#         label = label.reshape(len(label), 1) - 1
#         enc = OneHotEncoder()
#         a = enc.fit_transform(label)
